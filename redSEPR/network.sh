...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating CNE Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-cne.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating MOE Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-moe.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.cne.example.com cne
    peer_cert admin Admin@cne.example.com cne

    infoln "Creating MOE Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.moe.example.com moe
    peer_cert admin Admin@moe.example.com moe
  ...
  infoln "Generating CCP files for CNE and MOE"
...
# call the script to create the channel, join the peers of cne and moe,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to cne and moe, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    #EMAFLA ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.cne.example.com docker_peer0.moe.example.com
    ${CONTAINER_CLI} volume rm compose_orderer.example.com compose_peer0.cne.example.com compose_peer0.moe.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/moe/msp organizations/fabric-ca/moe/tls-cert.pem organizations/fabric-ca/moe/ca-cert.pem organizations/fabric-ca/moe/IssuerPublicKey organizations/fabric-ca/moe/IssuerRevocationPublicKey organizations/fabric-ca/moe/fabric-ca-server.db'
